...
def process_batch(batch, args):
    ...
    user_test_correct_answer = (user_test_correct_answer * mask).type(torch.FloatTensor)
    user_test_total_answer = (user_test_total_answer * mask).type(torch.FloatTensor)
    user_test_acc = (user_test_acc * mask).type(torch.FloatTensor)
    user_tag_correct_answer = (user_tag_correct_answer * mask).type(torch.FloatTensor)
    user_tag_total_answer = (user_tag_total_answer * mask).type(torch.FloatTensor)
    user_tag_acc = (user_tag_acc * mask).type(torch.FloatTensor)
    testid_first = (testid_first * mask).type(torch.FloatTensor)
    testid_rest = (testid_rest * mask).type(torch.FloatTensor)
    itemseq = (itemseq * mask).type(torch.FloatTensor)
    item_mean = (item_mean * mask).type(torch.FloatTensor)
    test_mean = (test_mean * mask).type(torch.FloatTensor)
    tag_mean = (tag_mean * mask).type(torch.FloatTensor)
    month = (month * mask).type(torch.FloatTensor)
    hour_bin = (hour_bin * mask).type(torch.FloatTensor)
    repeat = (repeat * mask).type(torch.FloatTensor)
    elapse_bin = (elapse_bin * mask).type(torch.FloatTensor)
    elapse_mean = (elapse_mean * mask).type(torch.FloatTensor)
    ...
    # device memory로 이동
    user_test_correct_answer = user_test_correct_answer.to(args.device)
    user_test_total_answer = user_test_total_answer.to(args.device)
    user_test_acc = user_test_acc.to(args.device)
    user_tag_correct_answer = user_tag_correct_answer.to(args.device)
    user_tag_total_answer = user_tag_total_answer.to(args.device)
    user_tag_acc = user_tag_acc.to(args.device)
    testid_first = testid_first.to(args.device)
    testid_rest = testid_rest.to(args.device)
    itemseq = itemseq.to(args.device)
    item_mean = item_mean.to(args.device)
    test_mean = test_mean.to(args.device)
    tag_mean = tag_mean.to(args.device)
    month = month.to(args.device)
    hour_bin = hour_bin.to(args.device)
    repeat = repeat.to(args.device)
    elapse_bin = elapse_bin.to(args.device)
    elapse_mean = elapse_mean.to(args.device)
