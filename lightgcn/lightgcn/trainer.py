...
from .utils import dropout_edge, dropout_path
...
def run(
    ...
    for e in range(n_epochs):
        ...
        train_auc, train_acc, train_loss = train(train_data=train_data, model=model, optimizer=optimizer, 
                                                 edge_dropout=edge_dropout, edge_dropout_rate=edge_dropout_rate,
                                                 path_dropout=path_dropout, path_dropout_rate=path_dropout_rate)
...
def train(model: nn.Module, train_data: dict, optimizer: torch.optim.Optimizer,
          edge_dropout: int, edge_dropout_rate: float, path_dropout: int, path_dropout_rate: float):
    
    if edge_dropout:
        masked_train_data, edge_mask=dropout_edge(
            train_data["edge"], p=edge_dropout_rate
        )
        
        pred = model(masked_train_data)
        loss = model.link_pred_loss(pred,
                                    torch.masked_select(torch.Tensor(train_data["label"]), edge_mask)
                                    )
    elif path_dropout:
        masked_train_data, edge_mask=dropout_path(
            train_data["edge"], p=path_dropout_rate
        )
    ...
    if edge_dropout or path_dropout:
        prob = model.predict_link(edge_index=masked_train_data, prob=True)
    else:
    ...
    if edge_dropout or path_dropout:
        label = torch.masked_select(torch.Tensor(train_data["label"]), edge_mask).cpu().numpy()
    else:
...
def inference(model: nn.Module, data: dict, output_dir: str):
    ...
    nowTime = nowDate.strftime("%Y-%m-%d_%H%M")
    os.makedirs(name=output_dir, exist_ok=True)
    write_path = os.path.join(output_dir, str(nowTime) + "submission.csv")
