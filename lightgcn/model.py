...
class ModelBase(nn.Module):
    def __init__(
        ...
        self.comb_proj = nn.Linear(intd * 5, hd) # 위에 4개라서 곱하기 4
        
        # Fully connected layer
        self.fc = nn.Linear(hd, 1)
        
        self.lgcn_linear = nn.Linear(128, intd)
        # self.embedding_user, self.embedding_item = get_embs()
        # self.embedding_user = self.embedding_user.to("cuda")
        # self.embedding_item = self.embedding_item.to("cuda")
        # print(self.embedding_user.size(), self.embedding_item.size())
        # torch.Size([7442, 64]) torch.Size([9454, 64])
        self.embed_matrix, self.num_users = get_embedding()
    
    def lgcn_embedding(self, item):
        item = item.detach().cpu().numpy()
        print(f"item: {item}")
        print(f"item_size: {len(item)}")
        embed_matrix, num_users = self.embed_matrix, self.num_users
        embed_matrix = embed_matrix.detach().cpu().numpy()
        
        len_users = num_users - 1

        item_embed = []
        for user in item:
            user_li = []
            for i in user:
                user_li.append(embed_matrix[len_users + i])
            item_embed.append(user_li)

        item_embed = torch.Tensor(np.array(item_embed))
        print(f"item_embed.size: {item_embed.size()}") # [64 20 128]
        return item_embed
    ...
    def forward(self, test, question, tag, correct, mask, interaction):
        ...
        embed_item = self.lgcn_embedding(question).to("cuda")
        embed_item = self.lgcn_linear(embed_item)
        print(embed_item.size())    # [64 20 21]
